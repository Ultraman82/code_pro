...
def plot_one_box_local(x, img, frame_file, q, color=None, label=None, line_thickness=3):
    ...
    instance_image = img[c1[1]:c2[1], c1[0]:c2[0]]
    q.put((f"{frame_file}/{label}.jpg", instance_image))
    if label:
...
def detect(model, dataset, output_dir):
    ...
    for index, (path, img, im0s, vid_cap, cur_frame) in enumerate(dataset):
        ...
        if not os.path.exists(instance_dir):  # output dir
            os.mkdir(instance_dir)
        # frame_file = instance_dir + '.jpg'
        ...
        pred = non_max_suppression(
            pred, 0.10, 0.45, classes=False, agnostic=False)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                frame_info['total_instance'] = len(det)
                ...
                for c in det[:, -1].unique():
                    ...
                    frame_info['classes'][class_name] = n

                # Write results
                for index_instance, (*xyxy, conf, cls) in enumerate(reversed(det)):
                    # label = f'{names[int(cls)]} {conf:.2f}'
                    label = f'{names[int(cls)]} {index_instance}'

                    plot_one_box_local(xyxy, im0, instance_dir, q, label=label,
                                       color=colors[int(cls)], line_thickness=3)

            q.put((instance_dir + '.jpg', im0))
            # print(f'{s}Done. ({t2 - t1:.3f}s)')
        yield(index, frame_info)
